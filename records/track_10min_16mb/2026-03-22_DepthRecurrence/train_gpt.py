...
class Hyperparameters:
    ...
    # Depth recurrence: run the blocks multiple passes to get effective depth > num_layers.
    num_recurrence_passes = int(os.environ.get("NUM_RECURRENCE_PASSES", 2))
...
class GPT(nn.Module):
    def __init__(
        ...
        # Per-pass skip weights so each recurrence pass can learn different skip blending.
        self.skip_weights = nn.Parameter(torch.ones(num_recurrence_passes, self.num_skip_weights, model_dim, dtype=torch.float32))
        ...
        # Learnable gate per pass (except the first) to blend recurrence output back.
        if num_recurrence_passes > 1:
            self.pass_gate = nn.Parameter(torch.zeros(num_recurrence_passes - 1, model_dim, dtype=torch.float32))
        else:
            self.pass_gate = None
    ...
    def _init_weights(self) -> None:
        ...
        # Scale output projections by effective depth (num_layers * num_passes).
        effective_depth = len(self.blocks) * self.num_recurrence_passes
    ...
    def _run_blocks(self, x: Tensor, x0: Tensor, pass_idx: int) -> Tensor:
        """Run one pass of encoder-decoder blocks with U-Net skip connections."""
        skips: list[Tensor] = []
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x, x0)
            skips.append(x)
        sw = self.skip_weights[pass_idx]
        for i in range(self.num_decoder_layers):
            if skips:
                x = x + sw[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x = self.blocks[self.num_encoder_layers + i](x, x0)
        return x

    def _run_recurrence(self, x: Tensor, x0: Tensor) -> Tensor:
        """Run all recurrence passes through the shared blocks."""
        x = self._run_blocks(x, x0, pass_idx=0)
        for p in range(1, self.num_recurrence_passes):
            x_new = self._run_blocks(x, x0, pass_idx=p)
            # Learnable sigmoid gate blends new pass output with previous.
            g = torch.sigmoid(self.pass_gate[p - 1].to(dtype=x.dtype))[None, None, :]
            x = (1 - g) * x + g * x_new
        return x
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_recurrence_passes=args.num_recurrence_passes,
    ).to(device).bfloat16()
